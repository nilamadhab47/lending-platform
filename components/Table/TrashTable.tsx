    ...
    renderTopToolbar: ({ table }) => {
        ...
        table.getSelectedRowModel().flatRows.map(async (row) => {
          ...
          try {
            const response = await fetch(
              `${process.env.NEXT_PUBLIC_DEV_URL}/ledger-service/trash/move-to-settled-invoice-from-trash/${sno}`,
              {
                method: "POST",
                headers: {
            ...
            if (response.ok) {
              ...
              toast.success("Put back to Settle Invoice"); // Dispatch the action with fetched data
            } else {
      ...
      const handleRaiseInvoice = () => {
        table.getSelectedRowModel().flatRows.map(async (row) => {
          ...
          try {
            const response = await fetch(
              `${process.env.NEXT_PUBLIC_DEV_URL}/ledger-service/trash/move-to-raised-invoice-from-trash/${sno}`,
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
          ...
          <Box>
            <Box sx={{ display: "flex", gap: "0.5rem" }}>
              ...
              <Menu
                ...
                onClose={handleClose}
              >
                <MenuItem onClick={handleActiveTrasaction}>
                  Active Transac
                </MenuItem>
                <MenuItem onClick={handleRaiseInvoice}>Raised Invo.</MenuItem>
                <MenuItem onClick={handleSettleInvoice}>Settled Invo.</MenuItem>
              </Menu>
            </Box>
          </Box>
        </Box>
...
const FinanceTableOfTrash = ({ data }: { data: RaisedInvoice[] }) => (
...
export default FinanceTableOfTrash;
