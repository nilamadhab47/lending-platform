...
const FinanceTable = ({data}:{data: RaisedInvoice[]}) => {
      ...
      table.getSelectedRowModel().flatRows.map(async (row) => {
        ...
        try {
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_DEV_URL}/ledger-service/move-raised-invoice-to-trash/${sno}`,
            {
              method: "DELETE",
              headers: {
  ...
  const handleCancel = () => {
    ...
    table.getSelectedRowModel().flatRows.map(async (row) => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_DEV_URL}/ledger-service/cancel-raised-invoice/${sno}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
    ...
    renderTopToolbar: ({ table }) => {
      ...
      const handleEditTransaction = () => {
        table.getSelectedRowModel().flatRows.map(async (row) => {
          ...
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_DEV_URL}/ledger-service/raised-invoice/${sno}`,
            {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
          ...
          if (response.ok) {
            const data = await response.json();
            console.log(data.raisedInvoice, "transaction data");
            dispatch(setEditRaisedInvoiceData(data.raisedInvoice));
            handleOpenModal();
      ...
      const handleDelete = () => {
        table.getSelectedRowModel().flatRows.map(async (row) => {
          const sno = row.getValue("sno");
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_DEV_URL}/ledger-service/move-to-settled-invoice/${sno}`,
            {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
          ...
          if (response.ok) {
            const data = await response.json();
            console.log(data.settled_invoice, "transaction data");
            dispatch(updateFormSubmitted(true));
            toast.success("move to TRASH") // Dispatch the action with fetched data
          } else {
            console.error("Failed to fetch data");
      ...
      const handleSettleInvoice = () => {
        table.getSelectedRowModel().flatRows.map(async (row) => {
          ...
          if (response.ok) {
            ...
            console.log(data.settled_invoice, "transaction data");
            dispatch(updateFormSubmitted(true));
            toast.success("move to settled invoice") // Dispatch the action with fetched data
          } else {
            console.error("Failed to fetch data");
          }
        });
      ...
      const handlePutBack = () => {
        table.getSelectedRowModel().flatRows.map(async (row) => {
          ...
          try {
            const response = await fetch(
              `${process.env.NEXT_PUBLIC_DEV_URL}/ledger-service/move-back-to-active-transaction/${sno}`,
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
            ...
            if (response.ok) {
              const data = await response.json();
              dispatch(updateFormSubmitted(true));
              toast.success("Put back to Active Transaction"); // Dispatch the action with fetched data
            } else {
              console.error("Failed to fetch data");
            }
          } catch (error) {
            ...
            console.log("finally put back to active Transaction");
          }
        });
      };
...
import RaisedInvoiceFormModal from "../Modal/RaisedInvoiceModal";
import { useDispatch } from "react-redux";
import { setEditRaisedInvoiceData, updateFormSubmitted } from "@/redux/slices/RaisedInvoiceForm";

const FinanceTableOfRaisedInvoice = ({data}:{data:RaisedInvoice[]}) => (
...
export default FinanceTableOfRaisedInvoice;
