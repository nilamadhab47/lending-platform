...
const FinanceTable = ({data}:{data: RaisedInvoice[]}) => {
    ...
    table.getSelectedRowModel().flatRows.map(async (row) => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_DEV_URL}/ledger-service/all-transactions/${sno}`,
          {
            method: "DELETE",
            headers: {
  ...
  const handleCancel = () => {
    ...
    table.getSelectedRowModel().flatRows.map(async (row) => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_DEV_URL}/ledger-service/cancel-transaction/${sno}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
    ...
    renderTopToolbar: ({ table }) => {
      ...
      const handleEditTransaction = () => {
        table.getSelectedRowModel().flatRows.map(async (row) => {
          ...
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_DEV_URL}/ledger-service/all-transaction/${sno}`,
            {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
          ...
          if (response.ok) {
            const data = await response.json();
            console.log(data.transaction, "transaction data");
            dispatch(setEditAllTransactionData(data.transaction));
            handleOpenModal();
      ...
      const handlePutBack = () => {
        table.getSelectedRowModel().flatRows.map((row) => {
          toast.success(
            "Cannot Put Back from all tranasaction " 
          );
        });
      };
...
import { setEditAllTransactionData, updateFormSubmitted } from "@/redux/slices/AllTransaction";
import AllTransactionFormModal from "../Modal/AllTranasactionModal";

const FinanceTableOfAllTransaction = ({data}:{data:RaisedInvoice[]}) => (
...
export default FinanceTableOfAllTransaction;
